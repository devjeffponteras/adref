...
export default function McdDashboard({ assetStatuses }: DashboardProps) {
    ...
    // Filter baselines natively mapped to match previous structural filters
    ...
    const rawEvaluated = useMemo(() => {
        return assetStatuses?.filter(item => !!item.asset?.mcd_information).length || 0;
    }, [assetStatuses]);

    const displayedTransactions = useMemo(() => {
        return assetStatuses?.filter(item => {
            const isAccountingApprovedTransaction = item?.asset?.accounting_information?.status === 'Approved';
            const hasMcdInfo = !!item.asset?.mcd_information;
            return (isAccountingApprovedTransaction && !hasMcdInfo) || hasMcdInfo;
        }) || [];
    }, [assetStatuses]);
    ...
    // Processing line: Sorting Active List Data
    ...
    return (
        <>
            ...
            <div className="container-fluid p-4">
                    ...
                    <div className="group relative overflow-hidden rounded-2xl border border-amber-100 bg-linear-to-br from-amber-50 to-orange-50/50 p-5 text-slate-800 shadow-xs transition-all duration-300 hover:-translate-y-1.5 hover:shadow-md hover:shadow-orange-500/5">
                        ...
                        <div className="flex justify-between items-start">
                            <div className="space-y-2">
                                ...
                                <h2 className="font-extrabold text-3xl tracking-tight text-amber-950">{rawPending.length || 0}</h2>
                            </div>
                    ...
                    <div className="group relative overflow-hidden rounded-2xl border border-emerald-100 bg-linear-to-br from-emerald-50 to-emerald-50/50 p-5 text-slate-800 shadow-xs transition-all duration-300 hover:-translate-y-1.5 hover:shadow-md hover:shadow-emerald-500/5">
                        ...
                        <div className="flex justify-between items-start">
                            <div className="space-y-2">
                                ...
                                <h2 className="font-extrabold text-3xl tracking-tight text-emerald-950">{rawEvaluated}</h2>
                            </div>
                ...
                {/* Main Data Table */}
                <div className="my-6 overflow-hidden rounded-2xl border border-zinc-100 shadow-xs bg-white">
                    <div className="overflow-x-auto">
                        <table className="w-full min-w-full divide-y divide-emerald-100/40 text-left align-middle text-sm">
                            <thead className="bg-zinc-100 text-xs font-bold uppercase tracking-wider text-slate-800 select-none">
                                <tr>
                                    <th scope="col" onClick={() => handleSort('control_number')} className="px-4 py-3.5 font-semibold cursor-pointer group hover:bg-zinc-200/60 transition-colors">
                                    ...
                                    <th scope="col" onClick={() => handleSort('submitted_by')} className="px-4 py-3.5 font-semibold cursor-pointer group hover:bg-zinc-200/60 transition-colors">
                                                ...
                                                <td className="py-4 pr-6 text-center whitespace-nowrap">
                                                    <Link 
                                                        ...
                                                        className={`inline-flex items-center gap-1.5 text-sm font-medium transition-colors px-3 py-2 rounded-lg
                                                            ${isLogged 
                                                                ? 'text-gray-500 hover:text-gray-700 hover:bg-gray-100/80 border border-gray-200 shadow-xs' 
                    ...
                    {/* Integrated Footer controls matching your request */}
                    {processedTransactions.length > 0 && (
                        <div className="px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-zinc-100 bg-zinc-50/50 select-none">
                            ...
                            <div className="flex items-center gap-1.5">
                                <div className="flex items-center gap-2">
                                    ...
                                    <select
                                        ...
                                        className="text-xs font-medium text-zinc-700 bg-white border border-zinc-200 rounded-lg px-2 py-1.5 outline-hidden focus:border-emerald-500 transition-all cursor-pointer shadow-xs"
                                    >
                                ...
                                <button
                                    ...
                                    className="p-2 text-zinc-500 bg-white border border-zinc-200 rounded-lg hover:bg-zinc-50 disabled:opacity-40 disabled:cursor-not-allowed shadow-xs transition-colors"
                                >
                                ...
                                <button
                                    ...
                                    className="p-2 text-slate-500 bg-white border border-zinc-200 rounded-lg hover:bg-zinc-50 disabled:opacity-40 disabled:cursor-not-allowed shadow-xs transition-colors"
                                >
